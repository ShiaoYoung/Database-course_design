...
// 获取账单列表函数
import { unsettledBillListService, unsettledAmountService } from '@/api/consume.js';
const billList = async () => {
    ...
    let result = await unsettledBillListService(params);
...
// 获取未结算金额
const unsettledAmount = async() => {
    let result = await unsettledAmountService();
...
<template>
    <el-card class="page-container">
        <template #header>
            <div class="header">
                <span>未结算总金额：<strong>{{ amount }}</strong>元。</span>
                <!-- <div class="extra">
        ...
        <el-table :data="bills" style="width: 100%">
            ...
            <el-table-column label="金额" prop="amount" align="right"></el-table-column>
            <!-- <el-table-column label="金额(元)" prop="amount" align="center">
                <template #default="{ row }">
                    <el-tag type="warning">{{ row.amount }}</el-tag>
                </template>
            </el-table-column> -->
